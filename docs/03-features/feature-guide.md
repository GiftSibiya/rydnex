...
- **Google sign-in:** After native sign-in (`@react-native-google-signin/google-signin`), the app posts `{ idToken }` to `POST /app-api/auth/auth/google-sign-in` via `authService.loginWithGoogle`. Login and register screens show success toasts (“Welcome back!” vs “Account created…”) based on `user.created_at` recency (or future `is_new_user` from the API). Global toasts use `ToastStateStore` + `PopUpWrapper` in `src/app/_layout.tsx`.
