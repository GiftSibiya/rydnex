...
- **Google sign-in:** After native sign-in (`@react-native-google-signin/google-signin`), the app posts `{ idToken }` to `POST /app-api/auth/auth/google-sign-in` via `authService.loginWithGoogle`. `GOOGLE_SIGN_IN_ENABLED` is `true` in `AppConfig.ts`; login and register screens show “Continue with Google”. Response shape matches password login (`user` + `session.accessToken`).
