...
export default function LogbookScreen() {
  ...
  const items: LogItem[] = useMemo(() => {
    ...
    const all: LogItem[] = [
      ...serviceLogs.filter((s) => s.vehicleId === activeVehicle.id).map((s) => ({ ...s, _type: "service" as const })),
      ...fuelLogs.filter((f) => f.vehicleId === activeVehicle.id).map((f) => ({ ...f, _type: "fuel" as const })),
      ...odometerLogs.filter((o) => o.vehicleId === activeVehicle.id).map((o) => ({ ...o, _type: "odometer" as const })),
    ];
    const filtered = filter === "All" ? all :
      filter === "Service" ? all.filter(i => i._type === "service") :
        filter === "Fuel" ? all.filter(i => i._type === "fuel") :
  ...
  const renderItem = ({ item }: { item: LogItem }) => {
    if (item._type === "service") {
      return (
        <View style={styles.entry}>
          <View style={[styles.entryLine, { backgroundColor: item.type === "repair" ? C.danger : C.tint }]} />
          <View style={[styles.entryIcon, { backgroundColor: item.type === "repair" ? "rgba(231,76,60,0.12)" : "rgba(46,204,113,0.1)" }]}>
            <Feather name="tool" size={14} color={item.type === "repair" ? C.danger : C.tint} />
          </View>
          <View style={styles.entryContent}>
            ...
            <Text style={styles.entryMeta}>{new Date(item.date).toLocaleDateString("en-ZA")} • {item.odometer.toLocaleString()} km{item.workshop ? ` • ${item.workshop}` : ""}</Text>
    ...
    if (item._type === "fuel") {
      return (
        <View style={styles.entry}>
          ...
          <View style={styles.entryContent}>
            <View style={styles.entryHeader}>
              <Text style={styles.entryTitle}>{item.liters}L @ R{item.costPerLiter.toFixed(2)}/L{item.fullTank ? " ✓" : ""}</Text>
              <Text style={styles.entryAmount}>R{item.totalCost.toFixed(0)}</Text>
            </View>
            <Text style={styles.entryMeta}>{new Date(item.date).toLocaleDateString("en-ZA")} • {item.odometer.toLocaleString()} km</Text>
          </View>
    ...
    return (
      <View style={styles.entry}>
        ...
        <View style={styles.entryContent}>
          <View style={styles.entryHeader}>
            <Text style={styles.entryTitle}>Odometer: {item.reading.toLocaleString()} km</Text>
          </View>
          <Text style={styles.entryMeta}>{new Date(item.date).toLocaleDateString("en-ZA")}{item.note ? ` • ${item.note}` : ""}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <View style={[styles.screen, { paddingTop: topPad }]}>
      ...
      <View style={styles.addButtons}>
        <TouchableOpacity style={styles.addQuick} onPress={() => router.push("/log/fuel")} activeOpacity={0.8}>
          <Feather name="droplet" size={14} color={C.info} />
          <Text style={[styles.addQuickText, { color: C.info }]}>Fuel</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.addQuick} onPress={() => router.push("/log/service")} activeOpacity={0.8}>
          <Feather name="tool" size={14} color={C.tint} />
          <Text style={[styles.addQuickText, { color: C.tint }]}>Service</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.addQuick} onPress={() => router.push("/log/odometer")} activeOpacity={0.8}>
          <Feather name="activity" size={14} color={C.success} />
      ...
      {items.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>Start logging fuel, services, or odometer readings</Text>
        </View>
      ) : (
        <FlatList
          data={items}
