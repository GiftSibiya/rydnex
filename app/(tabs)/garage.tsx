...
const MAKES = ["Audi", "BMW", "Ford", "Honda", "Hyundai", "Kia", "Land Rover", "Lexus", "Mazda", "Mercedes-Benz", "Mitsubishi", "Nissan", "Porsche", "Renault", "Subaru", "Suzuki", "Tesla", "Toyota", "Volkswagen", "Volvo"];
...
export default function GarageScreen() {
  ...
  const [form, setForm] = useState({
    make: "", model: "", year: String(currentYear),
    vin: "", registration: "", color: "", currentOdometer: "0",
  });
  ...
  const handleAdd = async () => {
    ...
    setShowAdd(false);
    setForm({ make: "", model: "", year: String(currentYear), vin: "", registration: "", color: "", currentOdometer: "0" });
    setErrors({});
  };
      ...
      >
        <View style={styles.topRow}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Feather name="plus" size={18} color={vehicles.length >= FREE_TIER_LIMIT ? C.textSubtle : C.tint} />
          </TouchableOpacity>
        </View>
        ...
        {vehicles.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <GoldButton label="Add Vehicle" onPress={() => setShowAdd(true)} style={{ marginTop: 8 }} />
          </View>
        ) : (
          vehicles.map((v) => (
            <TouchableOpacity
              key={v.id}
              activeOpacity={0.85}
              onPress={() => { setActiveVehicle(v); router.push("/"); }}
            >
              <LuxCard style={styles.vehicleCard}>
                <View style={styles.vehicleHeader}>
                  ...
                  <View style={styles.vehicleInfo}>
                    <Text style={styles.vehicleName}>{v.year} {v.make}</Text>
                    <Text style={styles.vehicleModel}>{v.model}</Text>
                  </View>
                  <TouchableOpacity onPress={() => handleDelete(v)} style={styles.delBtn} activeOpacity={0.7}>
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowAdd(false)}>
        ...
        >
          ...
          <View style={styles.formGrid}>
            <LuxInput label="Make" placeholder="e.g. Toyota" value={form.make} onChangeText={(t) => setForm(f => ({ ...f, make: t }))} error={errors.make} autoCapitalize="words" />
            <LuxInput label="Model" placeholder="e.g. Corolla" value={form.model} onChangeText={(t) => setForm(f => ({ ...f, model: t }))} error={errors.model} autoCapitalize="words" />
            <LuxInput label="Year" placeholder="e.g. 2021" value={form.year} onChangeText={(t) => setForm(f => ({ ...f, year: t }))} keyboardType="numeric" error={errors.year} />
            <LuxInput label="Registration" placeholder="e.g. CA123456" value={form.registration} onChangeText={(t) => setForm(f => ({ ...f, registration: t }))} autoCapitalize="characters" error={errors.registration} />
            <LuxInput label="VIN (optional)" placeholder="17-char VIN" value={form.vin} onChangeText={(t) => setForm(f => ({ ...f, vin: t }))} autoCapitalize="characters" />
            <LuxInput label="Color (optional)" placeholder="e.g. Pearl White" value={form.color} onChangeText={(t) => setForm(f => ({ ...f, color: t }))} autoCapitalize="words" />
            <LuxInput label="Current Odometer (km)" placeholder="0" value={form.currentOdometer} onChangeText={(t) => setForm(f => ({ ...f, currentOdometer: t }))} keyboardType="numeric" />
          </View>

          <GoldButton label="Save Vehicle" onPress={handleAdd} loading={loading} />
        </KeyboardAwareScrollView>
      </Modal>
    </View>
  );
