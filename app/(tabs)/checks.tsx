...
const CHECK_FIELDS = [
  { key: "oil" as const, label: "Engine Oil", icon: "droplet", tip: "Check every 2 weeks or before long trips" },
  { key: "tyrePressure" as const, label: "Tyre Pressure", icon: "disc", tip: "Check monthly or before long trips" },
  { key: "coolant" as const, label: "Coolant Level", icon: "thermometer", tip: "Check monthly when engine is cold" },
  { key: "spareWheel" as const, label: "Spare Wheel", icon: "circle", tip: "Check spare tyre pressure monthly" },
  { key: "lights" as const, label: "All Lights", icon: "zap", tip: "Check weekly — walk around your car" },
];

export default function ChecksScreen() {
  const { activeVehicle, lastChecks, updateLastCheck, vehicles } = useVehicle();
    ...
    >
      ...
      {!activeVehicle || vehicles.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>Add a vehicle to start tracking checks</Text>
        </View>
      ) : (
        <>
          {allGood && (
          ...
          <LuxCard style={styles.card}>
            <Text style={styles.cardTitle}>{activeVehicle.year} {activeVehicle.make} {activeVehicle.model}</Text>
          ...
          <LuxCard style={styles.tipsCard} elevated>
            ...
            {CHECK_FIELDS.map((f) => (
              <View key={f.key} style={styles.tipRow}>
                ...
                <Text style={styles.tipText}><Text style={styles.tipLabel}>{f.label}: </Text>{f.tip}</Text>
              </View>
            ))}
          </LuxCard>
        </>
